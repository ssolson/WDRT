...
class TestMLER(unittest.TestCase):
    ...
    def test_mler(self):
        ...
        RAOdir      = join(datadir, 'RAO_data')
        outputDir   = join(datadir, 'TestData')
        Test.readRAO(3,join(RAOdir,'RAO_heave_RM3float.dat'))
        Test.readRAO(5,join(RAOdir,'RAO_pitch_RM3float.dat'))
